module SRL.Interp ( runProgram, VarTab, varTabToString ) where
import SRL.Error
import SRL.AST
...
-- interpreting a program - CHANGED FROM RL
...
-- interpreting an instruction - CHANGED FROM RL
  ...
  If t b1 b2 a     -> do
    ...
    case bt of
      BoolValue bl1   -> do
        ...
        ba <- eval a
        case ba of
          BoolValue bl2 | bl1==bl2   -> return ()
                      | otherwise  -> throwError TestAndAssertionDoNotMatch
          _                        -> throwError AssertionNotBoolean
      _             -> throwError TestNotBoolean
  From a b t -> do
    ba <- eval a
    case ba of
      BoolValue True  -> nextIteration a b t --interpAST b
      BoolValue False -> throwError AssertionFalseMidLoop
      _             -> throwError AssertionNotBoolean
  Skip      -> return ()

nextIteration :: Expression -> [Statement] -> Expression -> ProgState ()
nextIteration a b t = do
  ...
  case bt of
    ...
    BoolValue False -> do
      ba <- eval a
      case ba of
        BoolValue False -> nextIteration a b t
        BoolValue True  -> error "Assertion evaluated to true mid-loop."
