...
The \textit{Common} module contains the implementation of the core features that are shared between the two languages.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{\textwidth}
    ...
    \caption{Partly implementation of binary operator expression evaluation.}\label{fig:exbinary}
  \end{subfigure}

  \begin{subfigure}{\textwidth}
    \lstinputlisting[language=haskell, firstline=288, lastline=293]{../src/src/Common/Interp.hs}
    \caption{Partly implementation of unary operator expression evaluation.}\label{fig:exunary}
  \end{subfigure}
...
\subsection{The \textit{RL} Interpreter}

In section~\ref{sec:rlandsrl} we considered the formal grammar (
...
\subsection{The \textit{SRL} Interpreter}
